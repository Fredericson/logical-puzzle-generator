...
Difficulty is owned by `DifficultyPolicy`: inspect only final visible constraints after clue reduction and count `FixedPositionConstraint` clues. Easy is >=2, Medium is exactly 1, Hard is 0. Do not put difficulty logic in the solver, validator, clue renderer, PDF generator, or translation catalog.
