...
PuzzleGenerator._derive_relational_constraints(solution)
        ↓
ConstraintDistributionPolicy accepts/scores fixed + relational constraints
...
Constraint distribution is a soft quality optimization. The generator should prefer varied mixes of the existing fixed-position, direct-left, left-of, direct-right, right-of, and adjacent constraints, while avoiding distributions dominated by one relation type. Easy, Medium, and Hard still derive correctness and displayed difficulty solely from final visible fixed-position counts plus unique-solution validation.
