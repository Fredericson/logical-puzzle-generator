...
When changing clue variety, use `ConstraintDistributionPolicy` for deterministic analysis, rejection, and scoring of existing constraints. Do not add clue types, move validation into the policy, or treat diversity as a correctness rule. Reduction may use the policy only to choose among otherwise valid removable-clue alternatives.
