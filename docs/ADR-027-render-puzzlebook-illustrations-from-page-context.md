...
Drawings use ReportLab vector primitives. Controlled visual variation uses PDF-only namespaces such as `puzzle_book.pdf.position` and `puzzle_book.pdf.theme_page.<page_index>.<category_id>`, derived with the existing stable seed helper rather than Python `hash()`.

PuzzleBook headers rely on naturally measured ReportLab paragraphs and flowables; illustrations are inserted as separate flowables below the header and instruction area, so long localized Question labels do not overlap artwork.
