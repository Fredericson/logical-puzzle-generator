...
@pytest.mark.parametrize(
    ...
    [
        ("tennis_training", "training", "forehand", "Mia trainiert die Vorhand.", "Mia practises the forehand."),
        ...
        ("athletics_training", "event", "pole_vault", "Mia übt Stabhochsprung.", "Mia practises pole vault."),
        ("zoo_visit", "animal_area", "crocodiles", "Mia besucht die Krokodile.", "Mia visits the crocodiles."),
    ],
)
def test_direct_assignment_clues_are_theme_specific_and_natural(theme_id, category_id, value_id, de_expected, en_expected) -> None:
    child = Item("Mia")
...
def test_generated_pdfs_do_not_expose_internal_theme_ids(tmp_path, language: str) -> None:
    puzzle = PuzzleGenerator(random_source=random.Random(4), difficulty="easy", theme="beach_day").generate(create_template())
