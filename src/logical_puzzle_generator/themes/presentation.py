...
from logical_puzzle_generator.themes.registry import ThemeDefinition
...
class ItemPresentationResolver:
    ...
    def theme_value(self, item: Item):
        ...
        return self.theme.value_by_id(item.name)
    ...
    def theme_subject_phrase(self, item: Item) -> str:
        ...
        template = self.theme.wording.theme_subject.for_language(self.language)
        return template.format(theme=value.display(self.language), theme_subject=value.subject(self.language))

    def child_with_theme_phrase(self, item: Item) -> str:
        value = self.theme_value(item)
        template = self.theme.wording.child_with_theme.for_language(self.language)
        return template.format(theme=value.display(self.language), theme_subject=value.subject(self.language))

    def direct_assignment_sentence(self, child: Item, theme_item: Item) -> str:
        template = self.theme.wording.direct_assignment.for_language(self.language)
