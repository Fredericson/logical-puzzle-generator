...
`ConstraintDistributionPolicy` analyzes generated constraints before clue text is created. It scores the deterministic distribution of existing supported clue types, rewards relation-type variety, and rejects obviously repetitive mixes such as all `LeftOfConstraint` or all `AdjacentConstraint` when a richer four-item relation set is available. This is a quality optimization only: solver and validator uniqueness checks remain the correctness authority, and no new clue or constraint types are introduced. The reducer uses the same score as a tie-breaker so equivalent removable clues prefer the final clue distribution that remains more balanced.
