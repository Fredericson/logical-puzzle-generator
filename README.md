...
Grammar-sensitive wording remains data-driven. Theme values carry localized short labels for answer rows, available-value boxes, summaries, and solution summaries, plus full labels and subject or position-anchor phrases for clues. `position_subject_phrase` is the normal default for generic position rendering; `position_anchor_sentence` is a narrow optional override only when complete sentence control is needed, such as plural strings, sunglasses, or footwork values that naturally use “belong to”. Every new production value is covered by direct, relative, and position presentation tests, and no category-specific rendering branch exists.
