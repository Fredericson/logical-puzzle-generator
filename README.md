...
Themed puzzles do not have a universal three-clue count. They are reduced while preserving unique solvability across child positions and the single thematic category, exact child-position anchor difficulty, one visible clue per mathematical constraint, and no hidden constraints. Position-only compatibility paths may still retain the original three-clue behavior.
